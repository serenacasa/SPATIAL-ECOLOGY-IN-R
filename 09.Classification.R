...
sun <- im.import("Solar_Orbiter_s_first_views_of_the_Sun_pillars.jpg")      # we chose the image of the sun from ESA
...
m1992 <- im.import("matogrosso_l5_1992219_lrg.jpg")                         # water is going to appear black as it absorbs the infrared
m2006 <- im.import("matogrosso_ast_2006209_lrg.jpg")                        # we are going to use only 2 clusters
...
par(mfrow=c(1,2))                                                          # to compare the two images, we can in this way clearly see their differences
plot(m1992C[[1]])
plot(m2006C[[1]])
...
f1992 <- freq(m1992C[[1]])
f1992                                                                      # 3 sets of the same values since we have three levels in the image

tot1992 <- ncell(m1992C[[1]])                                              # calculates the number of pixels in an image
tot1992

#### percentage of 1992 ####
p1992 <- f1992*100/tot1992
p1992                                                                       # the count section gives us the percentage, 
...
p2006                                                                      # the count tells us that the forest now covers the 45%, and the human he 54.7.%, so 55%

#### GRAPHIC ####
                                                                            # To make a graph of this, we should:
                                                                            ## 1. built a dataframe with the function data.frame:
                                                                              ### build the columns of the dataframe, using class()

class <- c("forest", "human")
y1992 <- c(83, 17)                                                           # y axis m1992 values obtained from the image percentage to have our data on the graph
y2006 <- c(45, 55)

tab <- data.frame(class, y1992, y2006)
tab

                                                                             ## 2. to build a graph we need the ggplot2 function
library(ggplot2)                                                             # aes = aestetics that manages the formation of 
                                                                             # the graph, values on the x axis, y axis and the color
...
p1 + p2
                                                                    ...
                                                                    # To have the same scale in each image graph 
                                                                    ## we have to use add the specification to each line --> + ylim(c(0,100))
