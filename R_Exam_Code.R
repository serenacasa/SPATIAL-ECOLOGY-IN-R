...
#### ----- BURN SEVERITY (dNDVI) -----
# Fire damage severity is assessed by calculating dNDVI across the entire burn perimeter
...
fire_impact.plot <- function() {
  ...
  plot(ndvi.nov.2018, col = ndvi_col, range = c(-1, 1),
       main = "Pre-fire NDVI\nNov 6, 2018", legend = TRUE, axes = FALSE)
  plot(fire_perimeter.utm, add = TRUE, border = "black", lwd = 1)
  
  plot(ndvi.dec.2018, col = ndvi_col, range = c(-1, 1),
       main = "Post-fire NDVI\nDec 6, 2018", legend = TRUE, axes = FALSE)
  plot(fire_perimeter.utm, add = TRUE, border = "black", lwd = 1)
  
  plot(dNDVI.18, col = fire.col, range = c(-0.5, 0.5),
       main = "dNDVI\n(fire impact)", legend = TRUE, axes = FALSE)
  plot(fire_perimeter.utm, add = TRUE, border = "black", lwd = 1) 
}

png("Images/Camp_fire_impact.png", width = 3200, height = 1400, res = 300)
fire_impact.plot()
dev.off()
...
dates <- c("Jul 2018","Nov 2018","Dec 2018","Jul 2019",
           "Jul 2020","Jul 2022","Jul 2024","Jul 2025")
...
temp.rec.maps <- function(){
  ...
  image.plot(legend.only = TRUE, zlim = c(-1, 1), 
             ...
             axis.args = list(at = seq(-1, 1, 0.5), 
                              labels = seq(-1, 1, 0.5), cex.axis = 1),
             legend.args = list(text = 'NDVI', side = 3, line = 0.9, cex = 0.85)) 
}

png("Images/NDVIs_Temporal_Series.png", width = 2700, height = 1800, res = 300)
temp.rec.maps()
dev.off()
...
cor_test <- cor.test(severity_vals, recovery_vals, method = "spearman", exact = FALSE)
cor_test$estimate
cor_test$p.value
...
severity_recovery <- function() {
  ...
  plot(recovery_cat, main = "Spatial Recovery Pattern",
       ...
       cex.main = 0.95, legend = FALSE)
  plot(fire_perimeter.utm, add = TRUE, border = "black", lwd = 1.5)
  mtext("Burn Severity × Recovery", side = 3, outer = TRUE, 
        line = 0.8, cex = 1.2, font = 2)
}

png("Images/Severity_x_Recovery.png", width = 3400, height = 1800, res = 300)
severity_recovery()
dev.off()
...
sd3_recovery <- focal(total_recovery, matrix(1/9, 3, 3), fun = sd)
sd5_recovery <- focal(total_recovery, matrix(1/25, 5, 5), fun = sd)
...
sd_variability <- function() {
  ...
  plot(sd7_recovery, main = "Local Variability (7×7)", col = sd_col, range = sd_range, 
       axes = FALSE, cex.main = 1)
  plot(fire_perimeter.utm, add = TRUE, border = "black", lwd = 1.5)
  mtext("Spatial Variability Analysis", side = 3, outer = TRUE, line = 0.8, cex = 1.4, 
        font = 2)
}

png("Images/SD_Recovery_Variability.png", width = 3000, height = 2400, res = 300)
sd_variability()
dev.off()
