...
#once packages are installed in R we don't need quotes "" anymore, install.packages(""), library()

#system.file function is going to catch a file and name it in the R system
file <-system.file("external/species.shp", package="sdm")
#shp is an extension of a file from a famous company. package= from which we want to take the file
...
rana$Occurrence
#these are called PRESENCE-ABSENCE data (1,0). This demonstrates only if that animal is there
#1 means a presence, but the 0 is an absence and is an uncertain data, since the "rana" could be there
...
#exercise: plot pres and abse altogether with two different colors

#to close the multiframe we have to use the dev.off() that causes graphical nulling
dev.off()
...
plot(pres, col=c1)
#PREDICTORS are environmental variables that describe why the rana temp is not present 
#where there are no dots.
